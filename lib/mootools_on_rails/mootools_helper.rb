module ActionView
  module Helpers
    module MootoolsHelper
     ...
     #   id = html_options.delete(:id) || "#{name.downcase.gsub(/\ /,"")[0..9]}#{Time.now.to_f.to_s.gsub(/\./,"")}" 
     #   function = block_given? ? update_page(&block) : args[0] || ''
     #   onclick = "#{"#{html_options[:onclick]}; " if html_options[:onclick]}#{function}; return false;"
     #   href = html_options[:href] || '#'
     #   result = ""
     #   result << javascript_tag(inline_add_event(id,'click',onclick))
     #   result << content_tag(:a, name, html_options.merge(:href => href,:id=>id))
     #   result
     # end
